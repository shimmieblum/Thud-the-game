...
class GameState:
    def __init__(self, grid=None, turn_number=1, previous_state=None, captured=None, turns_per_game=70, prev_action=None) -> None:
        ''' 
    ...
    def dwarves(self): return self.grid.get_piece_list(Piece.DWARF)
    
    def trolls(self): return self.grid.get_piece_list(Piece.TROLL)
    ...
    def get_capture_sets(self, end_loc, movetype)-> 'list[set]':
        if movetype == MoveType.DWARF_MOVE: return [set()]
        elif movetype == MoveType.DWARF_HURL: return [{end_loc}]
            ...
            x,y = end_loc
            full_set = powerset([(a,b) for (a,b) in self.grid.get_adj(x,y) if self.grid.get_piece(a,b) == Piece.DWARF])
            full_set.remove([])
    ...
    def __eq__(self, o: object) -> bool:
        if not isinstance(o, GameState): return False
        else: 
