...
#         '''    
    ...
    def update_info(self, game_number, best_of, wins, dwarf_player, troll_player, turn_number, game_length, prev_action):
    ...
    def display_details_panel(self, state:GameState):
        ...
        self.add_text_box(dwarf_text, (20,20), 250, 100 , box_fill=GUI.DWARF_COLOUR, text_colour=GUI.WHITE, autosize_text=True)
        
        troll_text = f'TROLLS\n{self.troll_name}\n{self.troll_agent_type}\n wins: {self.troll_wins}'
        
        self.add_text_box(troll_text,(w-250-20,20), 250, 100, box_fill=GUI.TROLL_COLOUR, text_colour=GUI.WHITE, autosize_text=True)
        ...
        self.add_text_box(action_text, (20,160), 250, 100, box_fill=GUI.GREY, text_colour=GUI.WHITE, autosize_text=True)
        ...
        self.add_text_box(info_text, (w-250-20, 160), 250, 100, box_fill=GUI.GREY, text_colour=GUI.WHITE, autosize_text=True)
    ...
    def create_grid(self, state:GameState, display_panel_height):
        ...
        for x in range(17):
            for y in range(17): 
                ...
                if t == Piece.EMPTY: c = square_colours[colour]
                elif t == Piece.DWARF: c = (0,0,255)
                elif t == Piece.TROLL: c = (255,0,0)
                else: c = (0,0,0)
                ...
                if x == 0 and y > 0: self.render_text(f'   {y} ', GUI.WHITE, top_left=(left,top), line_height=self.square_height)
                if y == 0 and x > 0: self.render_text(f'   {x} ', GUI.WHITE, top_left=(left,top+15), line_height=self.square_height)
    ...
    def get_coordinates(self, mouse_position):
        ...
        if x < 0 or x > 16 or y < 0 or y > 16: return -1,-1
        else: return x,y
    ...
    def highlight_squares(self, locations, colour):
        ...
        for x,y in locations:
            ...
            pg.draw.rect(self.surface, border_colour , pg.Rect(tlx, tly+self.square_height-b_width, self.square_width, b_width))
            pg.draw.rect(self.surface, border_colour , pg.Rect(tlx, tly, b_width, self.square_height))
            pg.draw.rect(self.surface, border_colour , pg.Rect(tlx+self.square_width-b_width, tly,  b_width, self.square_height))
    ...
    def add_text_box(self, text, top_left, width, height, box_fill, text_colour, font =pg.font.SysFont ,horizontal_alignment= Alignment.CENTRE, verticle_alignment=Alignment.MIDDLE, text_size=24, autosize_text=False):
        margin = 0 
        tlx,tly = top_left
        lines = text.splitlines()
        if autosize_text: text_block_height = height - margin
        else: text_block_height = min(text_size * len(lines), height - margin)
        ...
        for line in lines:
            ...
            if horizontal_alignment == Alignment.LEFT: ttlx = tlx + margin
            elif horizontal_alignment == Alignment.CENTRE: ttlx = tlx + (width - text_width)  // 2
            elif horizontal_alignment == Alignment.RIGHT: ttlx = tlx + (width - margin  - text_width) 
            line_number += 1
