...
class TestGrid(unittest.TestCase):
    ...
    def test_set_piece(self):
        ...
        for (x,y), (input_piece, e_piece, e_return_piece) in expected:
            ...
            self.assertEqual(e_piece, self.grid.get_piece(x,y), f'new piece {(x,y)}')
    ...
    def test_move_piece(self):
        ...
        for (from_x, from_y), (to_x, to_y), (e_from, e_to, e_return) in expected:
            ...
            self.assertEqual(from_piece, e_from, f'from piece: {(from_x, from_y)} -> {to_x, to_y}')
            self.assertEqual(to_piece, e_to,  f'to piece: {(from_x, from_y)} -> {to_x, to_y}')
            self.assertEqual(ret_piece, e_return,  f'return piece: {(from_x, from_y)} -> {to_x, to_y}')
    ...
    def test_piecelist_simple(self):
        ...
        for pieces, exp in expected_len: self.assertEqual(len(pieces), exp)
        ...
        for pieces, locs in some_locations:
            for loc in locs: self.assertIn(loc, pieces)
    ...
    def test_all_pieces_lists(self):
        ...
        for x,y in dwarves: self.grid.remove_piece(x,y)
        for x,y in trolls: self.grid.remove_piece(x,y)
        self.assertEqual(len(self.grid.get_piece_list(Piece.DWARF)), 0, 'all DWARF pieces removed')
        self.assertEqual(len(self.grid.get_piece_list(Piece.TROLL)), 0, 'all TROLL pieces removed')
        
        max_x, max_y = self.grid.dimensions
        bad = [Piece.TROLL, Piece.DWARF]
        for x in range(1,max_x+1):
            for y in range(1,max_y+1):
                self.assertNotIn(self.grid.get_piece(x, y), bad, f'check board is clear at {(x,y)}')
        
        
    def test_copy(self):
        new_grid = self.grid.deepcopy()
        self.assertEqual(self.grid.pieces, new_grid.pieces, 'pieces are the same')
        new_grid.set_piece(10,10,Piece.DWARF)
        self.assertNotEqual(self.grid.get_piece(10,10), new_grid.get_piece(10,10), 'change in one not in other')
    ...
    def test_grid_from_template(self):
        ...
        self.assertEqual(self.grid.dimensions, (3,3), 'dimensions are correct')
        ...
        self.assertEqual(self.grid.get_piece(1,1), Piece.EMPTY, 'template correct')
