...
class TestGameState(unittest.TestCase):
    ...
    def test_new_state(self):
        ...
        self.assertEqual(self.state.grid.get_piece(
            7, 1), Piece.DWARF, 'grid created correctly')
    ...
    def test_valid_dwarf_actions_simple_board(self):
        self.state.grid.board_from_template([
            ...
            [Piece.TROLL, Piece.TROLL, Piece.EMPTY,
                Piece.NON_PLAYABLE, Piece.DWARF]
        ])
    ...
    def test_invalid_dwarf_actions_simple_board(self):
        ...
        for action in invalid_actions:
            self.assertNotIn(action, all_valid_actions, action)
    ...
    def test_valid_troll_actions_simple_board(self):
        ...
        valid_actions = [
            ...
            Action((1, 4), (1, 2), {(1, 1), (2, 2),
                   (2, 1)}, MoveType.TROLL_SHOVE),
            Action((2, 3), (3, 3), {(2, 4)}, MoveType.TROLL_MOVE),
            Action((2, 3), (3, 3), {(2, 2)}, MoveType.TROLL_MOVE),
            Action((2, 3), (3, 2), {(2, 2), (2, 1)}, MoveType.TROLL_SHOVE),
            Action((2, 3), (3, 2), {(2, 2)}, MoveType.TROLL_SHOVE),
        ...
        valid_troll_actions = self.get_troll_actions()
    ...
    def get_dwarf_actions(self):
        """
        get all dwarf actions from state created by set up
        """
        self.state.turn = Piece.DWARF
        return self.state.valid_actions()

    def get_troll_actions(self):
        """
        get all troll actions from state created by set up
        """
        self.state.turn = Piece.TROLL
        return self.state.valid_actions()
    ...
    def test_invalid_troll_actions_simple_board(self):
        ...
        invalid_actions = [
            # can't capture that far
            Action((1, 4), (1, 3), {(3, 4)}, MoveType.TROLL_MOVE),
            # can't move that far
            Action((1, 4), (1, 2), {(2, 2)}, MoveType.TROLL_MOVE),
            Action((2, 3), (3, 3), {(3, 2)},
                   MoveType.TROLL_MOVE),  # empty location
            Action((4, 2), (4, 3), set(), MoveType.TROLL_SHOVE),
            Action((4, 2), (4, 2), set(), MoveType.TROLL_MOVE),
            # try to move onto troll
            Action((2, 5), (2, 4), {(2, 3)}, MoveType.TROLL_MOVE),
            # try to move onto dwarf
            Action((2, 3), (2, 2), {(2, 2)}, MoveType.TROLL_MOVE),
            Action((2, 3), (2, 2), {(2, 2)}, MoveType.TROLL_SHOVE)
        ]

        valid_actions = self.get_troll_actions()
        for action in invalid_actions:
    ...
    def test_dwarf_actions_full_board(self):
        ...
        for (sx, sy), (tx, ty) in moves:
            self.state.grid.move_piece(sx, sy, tx, ty)

        all_valid = self.get_dwarf_actions()

        for valid in valid_actions:
            self.assertIn(valid, all_valid)
        for invalid in invalid_actions:
            self.assertNotIn(invalid, all_valid)
    ...
    def test_troll_actions_full_board(self):
        ...
        all_valid = self.get_troll_actions()
