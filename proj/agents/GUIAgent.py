...
class GUIAgent(ThudAgentTemplate):
        ...
        while not turn_over:
            for event in pg.event.get():
                ...
                if event.type == pg.QUIT: 
                    pg.quit()
                    sys.exit()
                if self.state == State.COMPLETE: turn_over = True
                elif event != None:
    ...
    def from_loc(self, action:Action, game_state: GameState, game_number, wins, event)-> Action:
        if event.type != pg.MOUSEBUTTONUP: return action
    ...
    def to_loc(self, action:Action, game_state: GameState, game_number, wins, event)-> Action:
        ...
        if (fx,fy) == (x,y): 
            ...
            action.to_loc = x,y
            equiv_action = list(filter(lambda p: p.to_loc == (x,y), acceptable_moves))

            action.movetype = equiv_action[0].movetype
            if action.movetype == MoveType.DWARF_HURL: action.capture.add((x,y))
            self.change_state(action, State.CAPTURE_LIST, game_state)
        return action
    
    def capture_list(self, action:Action, game_state: GameState, game_number, wins, event)-> Action:
    
        if event.type == pg.KEYDOWN and event.key == K_RETURN:
            self.change_state(action, State.COMPLETE, game_state)
        if event.type != pg.MOUSEBUTTONUP and event.type : return action
        mouse_position = pg.mouse.get_pos()
        if self.gui.ok_button(mouse_position):
            ...
            if (x,y) == action.from_loc: 
                ...
                all_sets = game_state.get_capture_sets(action.to_loc, action.movetype)
    ...
    def change_state(self, action:Action, new_state, game_state:GameState):
        ...
        if new_state == State.FROM_LOC: 
            ...
            acceptable_moves = game_state.acceptable_moves(fx,fy)
            self.gui.display_grid(game_state)
            self.gui.highlight_squares([x.to_loc for x in acceptable_moves], ((176, 173, 5)))
            ...
            self.gui.display_grid(game_state)
            self.gui.highlight_squares([(fx,fy), (tx,ty)], (176, 0, 5)) # highlight to and from
            self.gui.highlight_squares(set(flatten(capture_sets)), ((176, 173, 5)))
            self.gui.highlight_squares([loc for loc in action.capture], (0, 0, 0)) # highlight locs in capture list
